...
void mouse_levels_handler() {
  ...
  if (x < 0) x = 0;
  else if (x + cursor_levels->sprite->width >= x_max) x = x_max - cursor_levels->sprite->width - 1;

  y -= mouse_packet.delta_y;
  if (y < 0) y = 0;
  else if (y + cursor_levels->sprite->height >= y_max) y = y_max - cursor_levels->sprite->height - 1;
